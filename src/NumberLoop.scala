...
object NumberLoop {
  ...
  private def calculateScoreWithExponentialMultiplier(
      args: util.ArrayList[Integer]
  ) = {
    var score = 0 // initialize the values
  ...
  def main(args: Array[String]): Unit = {
    ...
    val scores =
      new util.ArrayList[
        Integer
      ] // get the list to pass the methods with varargs
    ...
    while (!exit) {
      ...
      if (mode == 2) { // score multiplier mode - enter 1 to 9
        ...
        val numberStr =
          sc.next // get input and then get the first digit of integer input
