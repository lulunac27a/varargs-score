...
def calculate_score_with_exponential_multiplier(*args):
    ...
    for value in values:
        score += value * multiplier
        multiplier += multiplier / (1 + math.log(multiplier))#increase multiplier exponentially every time
    return score
...
if (mode == 1) | (mode == 2):
    ...
    while not exit_program:
        ...
        values_list.append(sum(int(digit) for digit in str(value_of)))#sum of all digits
        if value_of == 0:
            exit_program = True
...
if mode == 1:
    ...
    print("Score: " + str(math.floor(calculate_score_with_exponential_multiplier(*values_list))))
